...
class LoggedInTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # 各インスタンスメソッドで使うテスト用ユーザを生成しインスタンス変数に格納しておく
...
class TestDiaryCreateView(LoggedInTestCase):
    ...
    def test_create_diary_success(self):
        ...
        # 新規日記作成処理(Post)を実行
        responce = self.client.post(reverse_lazy('diary:diary_create'), params)

        # 日記リストページへのリダイレクトを検証
        self.assertRedirects(responce, reverse_lazy('diary:diary_list'))
    ...
    def test_create_diary_failure(self):
        ...
        # 新規日記作成処理(Post)を実行
        responce = self.client.post(reverse_lazy('diary:diary_create'))

        # 必須フォームフィールドが未入力によりエラーになることを検証
        self.assertFormError(responce, 'form', 'title', 'このフィールドは必須です。')
...
class TestDiaryUpdateView(LoggedInTestCase):
    ...
    def test_update_diary_success(self):
        ...
        self.assertRedirects(
            responce,
            reverse_lazy('diary:diary_detail', kwargs={'pk': diary.pk})
        )
